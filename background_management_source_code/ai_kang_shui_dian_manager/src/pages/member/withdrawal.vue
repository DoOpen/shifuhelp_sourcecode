<template>
  <div class="root">
    <toolbar title="提现列表" :toolData="toolData" @toolClick="toolClick"/>
  ...
  export default {
    ...
    beforeMount() {
      ...
      this.baseData = [
        ...
        {key: 'withdrawal_state_show', name: '提现状态'},
    ...
    methods: {
      getWithdrawalList(page){
        ...
        this.params.withdrawal_state=this.isNull(this.$route.params.withdrawal_state)?"":this.$route.params.withdrawal_state;
      ...
      operationClick(index,rowId){
        switch(index){
          case 0:
            this.$router.push('/withdrawal_editor/'+encodeURIComponent(JSON.stringify(this.withdrawalBeans[rowId])));
            break;
        }
      },
      toolClick(index){
    ...
    watch:{
      $route(to,from){
        this.page=1;
        this.getWithdrawalList(this.page);
      }
    }
  }
</script>
